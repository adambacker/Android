...
//	POIDATA_SERVER: "http://example.wikitude.com/GetSamplePois/",
    POIDATA_SERVER: "http://5.189.171.31:7033/api/wikitudes",
...
var World = {
	...
	loadPoisFromJsonData: function loadPoisFromJsonDataFn(poiData) {
		...
		World.markerDrawable_none = new AR.ImageResource("assets/icon_none.png");
		...
		for (var currentPlaceNr = 0; currentPlaceNr < poiData.length; currentPlaceNr++) {
            ...
            var currentDistance = World.getDistance(parseFloat(poiData[currentPlaceNr].latitude),
                                                                centerPointLatitude,
                                                                parseFloat(poiData[currentPlaceNr].longitude),
                                                                centerPointLongitude);
            var currentBearing = World.getBearing(parseFloat(poiData[currentPlaceNr].latitude),
                                                                centerPointLatitude,
                                                                parseFloat(poiData[currentPlaceNr].longitude),
                                                                centerPointLongitude);
		    for (var jj = 0; jj < poiData.length; jj++) {
		        if (currentPlaceNr == jj) continue;
		        var bearing_jj = World.getBearing(parseFloat(poiData[jj].latitude),
                                                        centerPointLatitude,
                                                        parseFloat(poiData[jj].longitude),
                                                        centerPointLongitude);
                var distance_jj = World.getDistance(parseFloat(poiData[jj].latitude),
                                                        centerPointLatitude,
                                                        parseFloat(poiData[jj].longitude),
                                                        centerPointLongitude);
                if (Math.abs(currentBearing - bearing_jj) < 5) {
                    if (currentDistance > distance_jj) {
                        show_flag = 0;
                        continue;
                    }
                }
		    ...
		    markerDrawable_idle = new AR.ImageResource(poiData[currentPlaceNr].image);

		    var distanceString = (currentDistance > 999) ? ((currentDistance / 1000).toFixed(2) + " km") : (Math.round(currentDistance) + " m");
			var singlePoi = {
...
//			poiData[i].distance = World.markerList[i].distanceToUser;
//            var distanceDouble = World.markerList[i].distanceToUser;
//            var distanceString = (distanceDouble > 999) ? ((distanceDouble / 1000).toFixed(2) + " km") : (Math.round(distanceDouble) + " m");

//			alert(World.markerList[i].distanceToUser);
		}
	},
	...
	plusDistance: function plusDistanceFn() {
	    distanceValue = parseFloat($("#panel-distance-range").val())+5;

        $("#panel-distance-range").val(distanceValue);
        $("#panel-distance-range").slider("refresh");
	},

    minusDistance: function minusDistanceFn() {
        distanceValue = parseFloat($("#panel-distance-range").val())-5;

        $("#panel-distance-range").val(distanceValue - 5);
        $("#panel-distance-range").slider("refresh");
    },
	/*
...
//			World.initiallyLoadedData = false;
	...
	requestDataFromServer: function requestDataFromServerFn(lat, lon) {
		...
		//http://example.wikitude.com/GetSamplePois/?lat=35&lon=120$nrPois=60
...
//		    "=20";
    ...
    getBearing: function (targetLatitude, centerPointLatitude, targetLongtitude, centerPointLongitude) {

            var x = Math.cos(targetLatitude * Math.PI / 180) * Math.sin((targetLongtitude - centerPointLongitude)*Math.PI / 180);
            var y = Math.cos(centerPointLatitude * Math.PI / 180)
                    * Math.sin(targetLatitude * Math.PI / 180)
                     - Math.sin(centerPointLatitude * Math.PI / 180)
                      * Math.cos(targetLatitude * Math.PI / 180)
                      * Math.cos((targetLongtitude - centerPointLongitude) * Math.PI /180);
